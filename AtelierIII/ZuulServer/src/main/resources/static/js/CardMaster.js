...
function achat(TransactionId) {
    var Name = new URLSearchParams(window.location.search).get("Name");
    var idUser = JSON.parse(loadRessource(`http://127.0.0.1:${port}/user/name/${Name}`, "GET")).id;
    var res = loadRessource(`http://127.0.0.1:${port}/market/buy/${TransactionId}/${idUser}`, "POST");
    if (res == "true") {
...
function room(RoomId, host) {
    ...
    if (Room.message == "" && host) {
        ...
        document.getElementById("content_body").innerHTML = loadRessource(`http://127.0.0.1:${port}/lobby/play/${RoomId}`, "GET");
        if (host) {
            loadRessource(`http://127.0.0.1:${port}/lobby/remove/${RoomId}`, "POST");
        }
        funReturnDefault();
    }
...
function join(CardId, RoomId) {
    ...
    var Name = new URLSearchParams(window.location.search).get("Name");
    var idUser = JSON.parse(loadRessource(`http://127.0.0.1:${port}/user/name/${Name}`, "GET")).id;
    loadRessource(`http://127.0.0.1:${port}/lobby/join/${RoomId}/${idUser}/${CardId}`, "PUT");
    var host = false;
    room(RoomId, host);
